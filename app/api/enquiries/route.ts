  ...
  try {
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        reply_to: data.email,
        subject: `New TAMTA Studios enquiry — ${data.company || "New project"}`,
        html: `
          <div style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;color:#1d1d1f">
            ...
            <p style="color:#6e6e73">Someone submitted the enquiry form on tamtastudios.</p>
            <hr style="border:0;border-top:1px solid #ddd;margin:24px 0" />
            <p><strong>Name</strong><br/>${escapeHtml(data.name)}</p>
            <p><strong>Email</strong><br/>${escapeHtml(data.email)}</p>
            <p><strong>Company</strong><br/>${escapeHtml(data.company || "—")}</p>
            <p><strong>Brief</strong><br/>${escapeHtml(data.brief).replace(/\n/g, "<br/>")}</p>
          </div>
        `,
      }),
    });
